...
std::optional<std::shared_ptr<User>> Repository::get_user_by_username(
...
std::optional<std::shared_ptr<TaskList>> Repository::get_task_list_by_name(
...
bool Repository::is_task_list_name_taken(
    const std::string& task_list_name) const {
    return task_lists.contains(task_list_name);
}

std::vector<std::shared_ptr<TaskList>> Repository::get_task_list_by_user(
